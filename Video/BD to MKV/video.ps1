...
if (Test-Path ($DemuxOutFolder + "EPs")) { 
    ...
    foreach ($video in $VideoList) {
        ...
        #$mytmp2 = ('--preset-import-file "1080p_x265.json" -Z "1080p x265" -i "' + $($video.Fullname) + '" -o "' + $($video2) + '"') 
        $mytmp2 = ('--preset-import-file "Star Trek TNG.json" -Z "Star Trek TNG" -i "' + $($video.Fullname) + '" -o "' + $($video2) + '"')
        Start-Process -FilePath $HandBrakeCLI -ArgumentList $mytmp2  -wait -NoNewWindow 
    }
}
...
if (Test-Path ($DemuxOutFolder + "SD")) { 
    ...
    foreach ($video in $VideoList) {
        ...
        $video2 = ($Destination + $video.Name)
        New-Item -ItemType "directory" -Path $Destination -ErrorAction SilentlyContinue | Out-Null
        $mytmp2 = ('--preset-import-file "d:\Video\SD_x264.json" -Z "SD x264" -i "' + $($video.Fullname) + '" -o "' + $($video2) + '"') 
        Start-Process -FilePath $HandBrakeCLI -ArgumentList $mytmp2  -wait -NoNewWindow 
    }
}
...
if (Test-Path ($DemuxOutFolder + "HD")) { 
    ...
    foreach ($video in $VideoList) {
        ...
        $video2 = ($Destination + $video.Name)
        New-Item -ItemType "directory" -Path $Destination -ErrorAction SilentlyContinue | Out-Null
        $mytmp2 = ('--preset-import-file "d:\Video\1080p_x265.json" -Z "1080p x265" -i "' + $($video.Fullname) + '" -o "' + $($video2) + '"') 
        Start-Process -FilePath $HandBrakeCLI -ArgumentList $mytmp2  -wait -NoNewWindow 
    }
}
...
if (Test-Path ($DemuxOutFolder + "4x3")) { 
    ...
    foreach ($video in $VideoList) {
        ...
        $video2 = ($Destination + $video.Name)
        New-Item -ItemType "directory" -Path $Destination -ErrorAction SilentlyContinue | Out-Null
        $mytmp2 = ('--preset-import-file "d:\Video\4x3_1080p_720p.json" -Z "4x3 1080p to 720p" -i "' + $($video.Fullname) + '" -o "' + $($video2) + '"') 
        Start-Process -FilePath $HandBrakeCLI -ArgumentList $mytmp2  -wait -NoNewWindow 
    }
}
