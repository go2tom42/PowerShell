...
if ($args.count -gt 0) {
    ...
    If ($args[0] -eq '50') {$fps = '50p'}
}

$MyPath = (Get-Location).path
$mkvmerge = 'C:\Program Files\MKVToolNix\mkvmerge.exe'

$VideoList = Get-ChildItem -Path ((Get-Location).path +'\video\*') -Include ("*.h264", "*.h265", "*.hevc", "*.divx", "*.avc", "*.mkv") -ErrorAction SilentlyContinue -Force | Sort-Object
...
$AudioList = Get-ChildItem -Path ((Get-Location).path +'\audio\*') -Include ("*.aac", "*.ac3", "*.dts", "*.eac3", "*.flac,", "*.mp1", "*.mp2", "*.mp3", "*.ogg", "*.mkv") -ErrorAction SilentlyContinue -Force | Sort-Object
...
$SubtitleList = Get-ChildItem -Path ((Get-Location).path +'\subtitles\*') -Include ("*.ass", "*.ssa", "*.srt", "*.sub", "*.sup") -ErrorAction SilentlyContinue -Force | Sort-Object
...
$Chapterlist = Get-ChildItem -Path ((Get-Location).path +'\chapters\*') -Include "*.txt" -ErrorAction SilentlyContinue -Force | Sort-Object
